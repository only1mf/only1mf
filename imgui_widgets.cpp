...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    ImRect bb( window->DC.CursorPos, window->DC.CursorPos + size_arg );
    ...
    float hovered_anim = ImTricks::Animations::FastFloatLerp( label, 0, hovered, 0.f, 1.f, ui::anim_speed );
    float held_anim = ImTricks::Animations::FastFloatLerp( label, 1, held, 0.f, 1.f, ui::anim_speed );
    auto col = ImTricks::Animations::FastColorLerp( ImTricks::Animations::FastColorLerp( GetColorU32( ImGuiCol_Button ), GetColorU32( ImGuiCol_ButtonHovered ), hovered_anim ), GetColorU32( ImGuiCol_ButtonActive ), held_anim );

    window->DrawList->AddRectFilled( bb.Min, bb.Max, col, style.FrameRounding );

    RenderText( bb.GetCenter( ) - label_size / 2, label );
...
ImRect ImGui::GetWindowScrollbarRect(ImGuiWindow* window, ImGuiAxis axis)
{
    ...
    if (axis == ImGuiAxis_X)
        ...
        return ImRect(ImMax(outer_rect.Min.x, outer_rect.Max.x - border_size - scrollbar_size / 2 - GetStyle( ).ItemSpacing.x / 2), inner_rect.Min.y + GetStyle( ).ItemSpacing.y, outer_rect.Max.x + scrollbar_size / 2 - GetStyle( ).ItemSpacing.x / 2, inner_rect.Max.y - GetStyle( ).ItemSpacing.y);
}
...
bool ImGui::Checkbox(const char* label, bool* v, float col[4])
{
    auto window = GetCurrentWindow( );
	auto id = window->GetID( label );
	auto style = GetStyle( );
	...
	ImRect bb( pos, pos + ImVec2( CalcItemWidth( ), square_sz ) );
	...
	auto bg_col = ImTricks::Animations::FastColorLerp( GetColorU32( ImGuiCol_FrameBg ), GetColorU32( ImGuiCol_Scheme ), enabled_anim );
	auto dot_col = ImTricks::Animations::FastColorLerp( GetColorU32( ImGuiCol_TextDisabled ), GetColorU32( ImGuiCol_Text ), enabled_anim );
	auto text_col = ImTricks::Animations::FastColorLerp( GetColorU32( ImGuiCol_TextDisabled ), GetColorU32( ImGuiCol_Text ), anim );
	...
	window->DrawList->AddText( { bb.Min.x, bb.GetCenter( ).y - label_size.y / 2 }, text_col, label, FindRenderedTextEnd( label ) );

    if ( col ) {
        pos = window->DC.CursorPos;
        window->DC.CursorPos = ImVec2{ bb.Max.x - square_sz * 3 - style.ItemInnerSpacing.x, bb.Min.y };

        BeginChild(  std::string( label ).append( "cb_child" ).c_str( ), { 24, square_sz }, 0, ImGuiWindowFlags_NoBackground );
        if ( ui::color_btn( std::string( label ).append( "cb" ).c_str( ), col, { 24, square_sz } ) ) {
            OpenPopup( std::string( label ).append( "popup" ).c_str( ) );
        }

        SetNextWindowSize( { 230, 0 } );
        PushStyleVar( ImGuiStyleVar_WindowPadding, { 16, 16 } );
        if ( BeginPopup( std::string( label ).append( "popup" ).c_str( ) ) ) {
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
	...
	ImVec2 pos = window->DC.CursorPos;
	ImVec2 label_size = CalcTextSize( label, 0, 1 );
	ImRect total_bb( pos, pos + ImVec2( CalcItemWidth( ), label_size.y + style.ItemInnerSpacing.y + GetFrameHeight( ) ) );
	...
	float rad = ImTricks::Animations::FastFloatLerp( label, 0, popup_open, IM_PI / 2, IM_PI * 1.5f, ui::anim_speed );
	float hovered_anim = ImTricks::Animations::FastFloatLerp( label, 1, hovered, 0.f, 1.f, ui::anim_speed );
	float popup_anim = ImTricks::Animations::FastFloatLerp( label, 2, popup_open, 0.f, 1.f, ui::anim_speed * 1.5f );
    float anim = ImTricks::Animations::FastFloatLerp( label, 3, hovered || popup_open, 0.f, 1.f, ui::anim_speed );
    auto col = ImTricks::Animations::FastColorLerp( GetColorU32( ImGuiCol_TextDisabled ), GetColorU32( ImGuiCol_Text ), anim );

	window->DrawList->AddRectFilled( bb.Min, bb.Max, GetColorU32( ImGuiCol_FrameBg ), style.FrameRounding, popup_open ? ImDrawFlags_RoundCornersTop : 0 );
    ...
    ui::rotate_start( );
    RenderArrow( window->DrawList, { bb.Max.x - style.FramePadding.x - 11, bb.GetCenter( ).y - 2.5f }, col, ImGuiDir_Down, 0.45f );
    ui::rotate_end( rad, ui::rotation_center( ) );

    window->DrawList->AddText( total_bb.Min, GetColorU32( ImGuiCol_Text ), label, FindRenderedTextEnd( label ) );

    window->DrawList->PushClipRect( bb.Min, bb.Max - ImVec2{ style.FramePadding.x - 35, 0 }, 1 );
    if ( preview_value )
        window->DrawList->AddText( bb.Min + style.FramePadding, col, preview_value, FindRenderedTextEnd( preview_value ) );
    ...
    if ( popup_open ) {
        ...
        GetWindowDrawList( )->AddRectFilled( GetWindowPos( ), GetWindowPos( ) + GetWindowSize( ), GetColorU32( ImGuiCol_PopupBg ), style.PopupRounding, ImDrawFlags_RoundCornersBottom );

        GImGui->CurrentWindow->SizeFull.x = bb.GetWidth( );
        GImGui->CurrentWindow->SizeFull.y = g.NextWindowData.SizeConstraintRect.GetHeight( ) * popup_anim;
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
	...
	ImVec2 pos = window->DC.CursorPos;
	ImVec2 label_size = CalcTextSize( label, 0, 1 );
	ImRect total_bb( pos, pos + ImVec2( CalcItemWidth( ), label_size.y + style.ItemInnerSpacing.y + 6 ) );
	ImRect bb( total_bb.Max - ImVec2( CalcItemWidth( ), 6 ), total_bb.Max );
	...
	ImRect grab_bb;
    const bool value_changed = SliderBehavior( bb, id, data_type, p_data, p_min, p_max, format, NULL, &grab_bb );
    if ( value_changed )
        MarkItemEdited( id );
    ...
    auto anim = ImTricks::Animations::FastFloatLerp( label, 0, hovered || IsItemActive( ), 0, 1, ui::anim_speed );
    auto col = ImTricks::Animations::FastColorLerp( GetColorU32( ImGuiCol_TextDisabled ), GetColorU32( ImGuiCol_Text ), anim );
	...
	window->DrawList->AddRectFilled( bb.Min, bb.Max, GetColorU32( ImGuiCol_FrameBg ), 2 );
	window->DrawList->AddRectFilled( bb.Min, { bb.Min.x + value->second * ui::content_anim, bb.Max.y }, GetColorU32( ImGuiCol_Scheme ), 2 );
    window->DrawList->AddCircleFilled( { bb.Min.x + value->second * ui::content_anim, bb.GetCenter( ).y }, 7, GetColorU32( ImGuiCol_SliderGrab ) );

	window->DrawList->AddText( total_bb.Min, col, label, FindRenderedTextEnd( label ) );
	window->DrawList->AddText( { total_bb.Max.x - CalcTextSize( value_buf ).x, total_bb.Min.y }, GetColorU32( ImGuiCol_Text ), value_buf );
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImVec2 total_size = frame_size + ImVec2{ 0, label_size.x > 0 ? style.ItemInnerSpacing.y + label_size.y : 0 };

    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + total_size);
    const ImRect frame_bb(total_bb.Max - frame_size, total_bb.Max);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton("##ColorButton", col_v4, flags | ImGuiColorEditFlags_NoTooltip | ImGuiColorEditFlags_NoBorder, { 18, 18 } ))
        {
    ...
    RenderText( { pos.x + 18 + style.ItemInnerSpacing.x, pos.y + 9 - GImGui->FontSize / 2 + 1 }, label );
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    float anim = ImTricks::Animations::FastFloatLerp( label, 0, hovered || selected, 0.f, 1.f, ui::anim_speed );
    float hovered_anim = ImTricks::Animations::FastFloatLerp( label, 1, hovered, 0.f, 1.f, ui::anim_speed );
    float selected_anim = ImTricks::Animations::FastFloatLerp( label, 2, selected, 0.f, 1.f, ui::anim_speed );
    auto col = ImTricks::Animations::FastColorLerp( ImTricks::Animations::FastColorLerp( GetColorU32( ImGuiCol_TextDisabled ), GetColorU32( ImGuiCol_Text ), hovered_anim ), GetColorU32( ImGuiCol_Scheme ), selected_anim );

    window->DrawList->AddRectFilled( bb.Min, bb.Max, ImColor{ 1.f, 1.f, 1.f, 0.03f * hovered_anim } );

    window->DrawList->AddText( { bb.Min.x + style.FramePadding.x, bb.GetCenter( ).y - label_size.y / 2 }, col, label );
